...
function bizz_post_content( $args = '', $post_count = false ) {
	...
	if ($args['remove_posts']=='0'){
		if ($args['full_posts']=='0' && ( is_archive() || $wp_query->is_posts_page || is_search() || is_home() )){
			the_excerpt();
			if ( $args['read_more'] )
				echo '<span class="read-more"><a href="' . get_permalink() . '" class="url fn" rel="nofollow">' . $args['read_more_text'] . '</a></span>';
		}
		else
...
function bizz_archive_headline() {
	global $wp_query; #wp
	$output = "<div class=\"headline_area archive_headline\">\n";
...
function bizz_post_meta($args = '') {
	...
	if ( is_singular() && !is_singular('post') ) {
		if ( isset($args['post_meta']) && $args['post_meta'] )
			$show_meta = true;
		else
			$show_meta = false;
	}
	...
	if ( $show_meta ) {
		...
		if ($args['post_date'])
			echo '<span class="date"><abbr class="published" title="' . get_the_time('Y-m-d') . '">' . get_the_time(get_option('date_format')) . '</abbr></span>';
		if ($args['post_author'])
		...
		if ($args['post_comments']) {	
			...
			comments_number(__('No Comments', 'bizzthemes'), __('One Comment', 'bizzthemes'), __('% Comments', 'bizzthemes'));
			echo '</a></span>';
		}
 ...
 * @since 6.0
 */
function bizz_get_image($key = 'image', $width = null, $height = null, $class = "thumbnail", $quality = 90,$id = null,$link = 'src',$repeat = 1,$offset = 0,$before = '', $after = '',$single = false, $force = false, $return = false, $is_auto_image = false, $src = '', $auto_meta = true, $meta = '', $cropp = '', $selflink = false, $filter = '', $sharpen = '' ) {
	// Run new function
	bizz_image( 'key='.$key.'&width='.$width.'&height='.$height.'&class='.$class.'&quality='.$quality.'&id='.$id.'&link='.$link.'&repeat='.$repeat.'&offset='.$offset.'&before='.$before.'&after='.$after.'&single='.$single.'&force='.$force.'&return='.$return.'&is_auto_image='.$is_auto_image.'&src='.$src.'&auto_meta='.$auto_meta.'&meta='.$meta.'&cropp='.$cropp.'&selflink='.$selflink.'&filter='.$filter.'&sharpen='.$sharpen );
	return;
}
	...
	*/
	...
	if ( isset($GLOBALS['opt']['bizzthemes_resize']['value']) && file_exists(TEMPLATEPATH . '/custom') ) {
		...
		foreach( $src_arr as $key => $custom_field ){
			...
			$img_rsource = BIZZ_FRAME_SCRIPTS . '/thumb.php?src='. $custom_field .'&amp;w='. $width .'&amp;h='. $height .'&amp;a='. $acrop . $filter . $sharpen .'&amp;zc=1&amp;q='. $quality;
