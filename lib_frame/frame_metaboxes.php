		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'date_time':
					...
					break;
				case 'text_money':
					echo '$ <input class="bizz_text_money" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', $meta ? $meta : $field['std'], '" /><span class="bizz_metabox_description">', $field['desc'], '</span>';
					break;
				case 'textarea':
				...
				case 'file_list':
					...
					echo '<input class="upload_button button" type="button" value="'.__('Upload File', 'bizzthemes').'" />';
					echo '<p class="bizz_metabox_description">', $field['desc'], '</p>';
						$args = array(
							...
							if ($attachments) {
								...
								foreach ($attachments as $attachment) {
									echo '<li>'.wp_get_attachment_link($attachment->ID, 'thumbnail', 0, 0, __('Download', 'bizzthemes'));
									echo '<span>';
					...
					echo '<div id="', $field['id'], '_status" class="bizz_upload_status">';	
						if ( $meta != '' ) { 
							...
							if ( $check_image ) {
								...
								echo '<a href="#" class="remove_file_button" title="'.__('Remove file', 'bizzthemes').'" rel="', $field['id'], '">'.__('Remove file', 'bizzthemes').'</a>';
								echo '</div>';
							} else {
								$parts = explode( "/", $meta );
								...
								echo ''.__('File:', 'bizzthemes').' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta, '" target="_blank" rel="external">'.__('Download', 'bizzthemes').'</a> / <a href="# title="'.__('Remove', 'bizzthemes').'" class="remove_file_button" rel="', $field['id'], '">'.__('Remove', 'bizzthemes').'</a>)';
							}	
						}
					echo '</div>'; 
				break;
