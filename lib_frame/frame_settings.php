...
function bizzthemes_add_submenus() {
	...
	if ( $adminmenu_layout != 'true' )
		add_submenu_page($menu_function, $themename, __('Builder', 'bizzthemes'), 'manage_options', 'bizz-layout','bizzthemes_layout');
		
	add_submenu_page($menu_function, $themename, __('Framework', 'bizzthemes'), 'manage_options', 'bizzthemes','bizzthemes_options');
	
	if ( $adminmenu_design != 'true' )
	    add_submenu_page($menu_function, $themename, __('Design', 'bizzthemes'), 'manage_options', 'bizz-design','bizzthemes_options');
		
	if ( $adminmenu_editor != 'true' )
	    add_submenu_page($menu_function, $themename, __('Editor', 'bizzthemes'), 'manage_options', 'bizz-editor', array('bizz_custom_editor', 'bizzthemes_editor'));
	
	if ( $adminmenu_license != 'true' && ( isset($_GET['page']) && $_GET['page'] == 'bizz-license' || $pending_license != '' ) )
	    add_submenu_page($menu_function, $themename, sprintf(__('License<span id="awaiting-mod" class="update-plugins"><span class="pending-count update-count">%s</span></span>', 'bizzthemes'), $pending_license), 'manage_options', 'bizz-license','bizzthemes_license_page');
	
	if ( $adminmenu_tools != 'true' && isset($_GET['page']) && $_GET['page'] == 'bizz-tools' )
	    add_submenu_page($menu_function, $themename, __('Backup', 'bizzthemes'), 'manage_options', 'bizz-tools', array('bizz_custom_tools', 'bizzthemes_tools'));
	
	if ( $adminmenu_version != 'true' && ( isset($_GET['page']) && $_GET['page'] == 'bizz-update' || $pending_update != '' ) )
	    add_submenu_page($menu_function, $themename, sprintf(__('Update<span id="awaiting-mod" class="update-plugins"><span class="pending-count update-count">%s</span></span>', 'bizzthemes'), $pending_update), 'manage_options', 'bizz-update','bizzthemes_framework_update_page');
...
<?php
				...
				if ( $sav != "") { $val = $sav; } elseif ( $sav2 != "") { $val = $sav2; } else { $val = $std; }
?>
				<input class="text_q q8 color {hash:true,caps:false,required:false}" name="<?php echo $value['id']; ?>[background-color]" id="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" value="<?php echo $val; ?>" />
				<input class="text_input" name="<?php echo $value['id']; ?>[css]" id="<?php echo $value['id']; ?>_css" type="hidden" value="<?php echo stripslashes(stripslashes($value['std']['css'])); ?>" />
<?php		
				// bg-repeat
...
<?php
			...
			case "color":
				...
				if ( $sav != "") { $val = $sav; } elseif ( $sav2 != "") { $val = $sav2; } else { $val = $std; }
?>
				<input class="text_q q8 color {hash:true,caps:false,required:false}" name="<?php echo $value['id']; ?>[color]" id="<?php echo $value['id']; ?>_color" type="<?php echo $value['type']; ?>" value="<?php echo $val; ?>" />
				<input class="text_input" name="<?php echo $value['id']; ?>[css]" id="<?php echo $value['id']; ?>_css" type="hidden" value="<?php echo stripslashes(stripslashes($value['std']['css'])); ?>" />
<?php
				option_wrapper_footer($value);
			break;
			...
			case "background-color":
				...
				if ( $sav != "") { $val = $sav; } elseif ( $sav2 != "") { $val = $sav2; } else { $val = $std; }
?>
				<input class="text_q q8 color {hash:true,caps:false,required:false}" name="<?php echo $value['id']; ?>[background-color]" id="<?php echo $value['id']; ?>_background-color" type="<?php echo $value['type']; ?>" value="<?php echo $val; ?>" />
				<input class="text_input" name="<?php echo $value['id']; ?>[css]" id="<?php echo $value['id']; ?>_css" type="hidden" value="<?php echo stripslashes(stripslashes($value['std']['css'])); ?>" />
<?php
				option_wrapper_footer($value);
			break;
...
function bizzthemes_options_header( $options_title = 'options', $toggle = false ) {
    global $options, $design, $bloghomeurl;
	...
	$return .= '<a href="#" class="feedback"  onclick="window.open(\'http://bizzthemes.wufoo.com/forms/m7x4a3/\',  null, \'height=560, width=670, toolbar=0, location=0, status=1, scrollbars=1,resizable=1\'); return false" title="'.__('Give your Feedback', 'bizzthemes').'">'.__('Report a Bug', 'bizzthemes').'&nbsp;&nbsp;<img src="'. BIZZ_FRAME_IMAGES . '/bug.png" width="22" height="28" alt="Feedback" /></a>'."\n";
	$return .= '</h2>'."\n";
	...
	if (!file_exists(TEMPLATEPATH . '/custom')) #Custom file alert
	    $return .= '<div id="message" class="updated fade themeinfo">'.__('<p>Rename your <code>custom-sample</code> folder to <code>custom</code>, otherwise theme will not work properly.</p>', 'bizzthemes').'</div>'."\n";
	if (file_exists(TEMPLATEPATH . '/custom') && !is_writable(TEMPLATEPATH . '/custom'))
	    $return .= '<div id="message" class="updated fade themeinfo">'.__('<p>Warning: Your <code>custom</code> folder is not writeable. Please <a href="http://codex.wordpress.org/Changing_File_Permissions">CHMOD</a> all custom folder files to 777 restrictions, otherwise theme will not work properly.</p>', 'bizzthemes').'</div>'."\n";
    if ( isset($_REQUEST['reset']) ) #Settings reset alert
	    $return .= '<div id="message" class="updated fade themeinfo">'.sprintf(__('<p><strong>Settings reset!&nbsp; <a href="%1$s">Check out your blog &rarr;</a></strong></p>', 'bizzthemes'), $bloghomeurl).'</div>'."\n";
...
function bizzthemes_options_footer() {
	...
	if ( $_GET['page'] == 'bizz-design' || $_GET['page'] == 'bizzthemes' ){
		...
		$return .= '<input name="save" type="submit" class="save_button" value="'.__('Save changes', 'bizzthemes').'" />'."\n";
		$return .= '</div>'."\n";
	    $return .= '</form>'."\n";
	    $return .= '<form method="post">'."\n";
		$return .= '<div class="reset_save reset">'."\n";
		$return .= '<input name="reset" type="submit" class="save_button" value="'.__('Reset', 'bizzthemes').'" onclick="return confirm(\''.__('All theme settings will be lost! Click OK to reset.', 'bizzthemes').'\');" />'."\n";
    ...
    echo '<div class="table-row">'."\n";
		...
		echo '<div class="top-container">'."\n";
			...
			if ( isset($values['desc']) ) {
			    ...
			    echo '<span class="trigger">'.__('more info [&#43;]', 'bizzthemes').'</span>'."\n";
