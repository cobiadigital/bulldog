...
function bizzthemes_framework_update_head(){
    ...
    if( isset($_REQUEST['page']) && $_REQUEST['page'] == 'bizz-update' ){
		...
		if( isset($_REQUEST['bizz_update_save']) && $_REQUEST['bizz_update_save'] == 'save' ){
			...
			if ( isset($_REQUEST['bizz_update_theme']) && $_REQUEST['bizz_update_theme'] == 'upgrade' ){
			...
			elseif ( isset($_REQUEST['bizz_update_frame']) && $_REQUEST['bizz_update_frame'] == 'upgrade' ){
				delete_transient('remote_f_version_' . $themeid);
...
function bizzthemes_framework_update_page(){
	...
	elseif ( $bizz_package == 'ZnJlZQ==' ) {
		...
		echo "\t\t<span style=\"display:none\">" . $method . "</span>\n";
			...
			echo "\t\t\t<div class=\"bizzalert_true\"><p>" . __("To use automatic framework updater, please <a href='" . $bloghomeurl . "wp-admin/admin.php?page=bizz-license'>Upgrade to Standard or Agency Theme Package</a>.", 'bizzthemes') . "</p></div>\n";
		...
		echo "\t\t<form method=\"post\"  enctype=\"multipart/form-data\" id=\"bizzform\" action=\"\">\n";
			...
			$remote_changelog = 'http://bizzthemes.com/demo/'.strtolower($themeid).'/wp-content/themes/'.strtolower($themeid).'/lib_theme/changelog.txt';
            ...
            if($this_theme_version < $remote_theme_version && $remote_theme_version != 'Currently Unavailable') {
				...
				echo "\t\t\t</ol>\n";
				echo '<input type="submit" class="button" value="'.__('Update Theme', 'bizzthemes').'" onclick="return confirm(\''.__('Old theme files will be lost! Click OK to proceed.', 'bizzthemes').'\');" />'."\n";
