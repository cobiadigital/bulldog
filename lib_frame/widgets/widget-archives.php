...
class Bizz_Widget_Archives extends WP_Widget {
	...
	function Bizz_Widget_Archives() {
		$widget_ops = array( 'classname' => 'archives', 'description' => __( 'Control the output of your archives.', 'bizzthemes' ) );
		$control_ops = array( 'width' => 525, 'height' => 350, 'id_base' => "widgets-reloaded-bizz-archives" );
		$this->WP_Widget( "widgets-reloaded-bizz-archives", __( 'Archives', 'bizzthemes' ), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( 'option' == $args['format'] ) {
			...
			if ( 'yearly' == $args['type'] )
				$option_title = __( 'Select Year', 'bizzthemes' );
			elseif ( 'monthly' == $args['type'] )
				$option_title = __( 'Select Month', 'bizzthemes' );
			elseif ( 'weekly' == $args['type'] )
				$option_title = __( 'Select Week', 'bizzthemes' );
			elseif ( 'daily' == $args['type'] )
				$option_title = __( 'Select Day', 'bizzthemes' );
			elseif ( 'postbypost' == $args['type'] || 'alpha' == $args['type'] )
				$option_title = __( 'Select Post', 'bizzthemes' );
	...
	function form( $instance ) {
		...
		$type = array( 'alpha' => __( 'Alphabetical', 'bizzthemes' ), 'daily' => __( 'Daily', 'bizzthemes' ), 'monthly' => __( 'Monthly', 'bizzthemes' ),'postbypost' => __( 'Post By Post', 'bizzthemes' ), 'weekly' => __( 'Weekly', 'bizzthemes' ), 'yearly' => __( 'Yearly', 'bizzthemes' ) );
		$format = array( 'custom' => __( 'Custom', 'bizzthemes' ), 'html' => __( 'HTML', 'bizzthemes' ), 'option' => __( 'Option', 'bizzthemes' ) );
		...
		<p>
			...
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_post_count'], true ); ?> id="<?php echo $this->get_field_id( 'show_post_count' ); ?>" name="<?php echo $this->get_field_name( 'show_post_count' ); ?>" /> <?php _e( 'Show post count?', 'bizzthemes' ); ?></label>
		</p>
		</div>
