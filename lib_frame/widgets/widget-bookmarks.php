...
class Bizz_Widget_Bookmarks extends WP_Widget {
	...
	function Bizz_Widget_Bookmarks() {
		$widget_ops = array( 'classname' => 'bookmarks', 'description' => __( 'Control the output of your links.', 'bizzthemes' ) );
		$control_ops = array( 'width' => 550, 'height' => 350, 'id_base' => "widgets-reloaded-bizz-bookmarks" );
		$this->WP_Widget( "widgets-reloaded-bizz-bookmarks", __( 'Links', 'bizzthemes' ), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		//Defaults
		$defaults = array(
			'title_li' => __( 'Bookmarks', 'bizzthemes' ),
			'category_order' => 'ASC',
		...
		$category_order = array( 'ASC' => __( 'Ascending', 'bizzthemes' ), 'DESC' => __( 'Descending', 'bizzthemes' ) );
		$category_orderby = array( 'count' => __( 'Count', 'bizzthemes' ), 'ID' => __( 'ID', 'bizzthemes' ), 'name' => __( 'Name', 'bizzthemes' ), 'slug' => __( 'Slug', 'bizzthemes' ) );
		$order = array( 'ASC' => __( 'Ascending', 'bizzthemes' ), 'DESC' => __( 'Descending', 'bizzthemes' ) );
		$orderby = array( 'id' => __( 'ID', 'bizzthemes' ), 'description' => __( 'Description',  'bizzthemes' ), 'length' => __( 'Length',  'bizzthemes' ), 'name' => __( 'Name',  'bizzthemes' ), 'notes' => __( 'Notes',  'bizzthemes' ), 'owner' => __( 'Owner',  'bizzthemes' ), 'rand' => __( 'Random',  'bizzthemes' ), 'rating' => __( 'Rating',  'bizzthemes' ), 'rel' => __( 'Rel',  'bizzthemes' ), 'rss' => __( 'RSS',  'bizzthemes' ), 'target' => __( 'Target',  'bizzthemes' ), 'updated' => __( 'Updated',  'bizzthemes' ), 'url' => __( 'URL',  'bizzthemes' ) );

		?>

		<div class="bizz-widget-controls columns-3">
		<p>
			<label for="<?php echo $this->get_field_id( 'title_li' ); ?>"><?php _e( 'Title:', 'bizzthemes' ); ?></label>
		...
		<p>
			...
			<input class="checkbox" type="checkbox" <?php checked( $instance['categorize'], true ); ?> id="<?php echo $this->get_field_id( 'categorize' ); ?>" name="<?php echo $this->get_field_name( 'categorize' ); ?>" /> <?php _e( 'Categorize?', 'bizzthemes' ); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'show_description' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_description'], true ); ?> id="<?php echo $this->get_field_id( 'show_description' ); ?>" name="<?php echo $this->get_field_name( 'show_description' ); ?>" /> <?php _e( 'Show description?', 'bizzthemes' ); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'hide_invisible' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['hide_invisible'], true ); ?> id="<?php echo $this->get_field_id( 'hide_invisible' ); ?>" name="<?php echo $this->get_field_name( 'hide_invisible' ); ?>" /> <?php _e( 'Hide invisible?', 'bizzthemes' ); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'show_rating' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_rating'], true ); ?> id="<?php echo $this->get_field_id( 'show_rating' ); ?>" name="<?php echo $this->get_field_name( 'show_rating' ); ?>" /> <?php _e( 'Show rating?', 'bizzthemes' ); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'show_updated' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_updated'], true ); ?> id="<?php echo $this->get_field_id( 'show_updated' ); ?>" name="<?php echo $this->get_field_name( 'show_updated' ); ?>" /> <?php _e( 'Show updated?', 'bizzthemes' ); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'show_images' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_images'], true ); ?> id="<?php echo $this->get_field_id( 'show_images' ); ?>" name="<?php echo $this->get_field_name( 'show_images' ); ?>" /> <?php _e( 'Show images?', 'bizzthemes' ); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'show_name' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_name'], true ); ?> id="<?php echo $this->get_field_id( 'show_name' ); ?>" name="<?php echo $this->get_field_name( 'show_name' ); ?>" /> <?php _e( 'Show name?', 'bizzthemes' ); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'show_private' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_private'], true ); ?> id="<?php echo $this->get_field_id( 'show_private' ); ?>" name="<?php echo $this->get_field_name( 'show_private' ); ?>" /> <?php _e( 'Show private?', 'bizzthemes' ); ?></label>
		</p>
